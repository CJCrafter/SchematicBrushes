...
public class SeparatedConfig implements Configuration {
    ...
    public void add(Map<String, ?> map, boolean isIgnoreDuplicates) {
        ...
        if (!isIgnoreDuplicates) {
            ...
            new LinkedHashSet<>(map.keySet()).forEach(key -> {
                if (containsKey(key)) {
                    DebugUtils.log(Log.ERROR, "Duplicate key \"" + key + "\"");
                    map.remove(key);
                }
            });
        }
    ...
    public void add(ConfigurationSection file, boolean isIgnoreDuplicates) {
        ...
        if (!isIgnoreDuplicates) {
            keys = keys.stream()
                    .filter(key -> {
                        if (containsKey(key)) {
                            DebugUtils.log(Log.ERROR, "Duplicate key \"" + key + "\" found in " + file.getName());
                            return false;
                        } else return true;
                    })
    ...
    public void set(String key, Object value) {
        ...
        else if (value instanceof Collection<?>) lists.put(key, convertSet(value));
    ...
    public void save(@Nonnull File file) {
        ...
        try {
            ...
            DebugUtils.log(Log.ERROR, "Could not save file \"" + file.getName() + "\"", ex);
        }
    }
}